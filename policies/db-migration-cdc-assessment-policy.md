...
이 정책은 전체 DB CDC 재시도 계획이 아니다. **기본: 2주**(평가 시작일 기준) 안에 확인 가능한 범위에서 1차 대상을 선별하고, IDC MSSQL 안정성과 성능 개선에 직접 영향을 줄 수 있는 batch, procedure, table, query를 찾기 위한 최소 기준이다.
...
전체 DB를 한 번에 조사하지 않는다. **기본: 2주**(평가 시작일 기준) 안에 확인 가능한 범위에서 아래 순서로 1차 대상을 고른다 — 범위가 크면 기간을 조정한다.
