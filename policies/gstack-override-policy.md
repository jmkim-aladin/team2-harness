...
### 커밋 메시지 품질 (Co-Authored-By 등)
- gstack 기본: 최종 커밋에 AI co-author 푸터 삽입
- **팀 규칙**: [ai-usage-policy.md](./ai-usage-policy.md) §메시지 작성 품질 — `/ship` 등 모든 커밋에서 제거
...
- 자동 배포 스킬은 merge까지만 수행하고 프로덕션 배포는 사람에게 위임한다
...
비활성 스킬을 되살릴 경우에도 같은 오버라이드가 그대로 적용된다.
