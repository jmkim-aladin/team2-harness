...
- **위키**는 항상 로컬 Obsidian vault(`$LOCAL_WIKI_PATH`)를 뜻한다. 사용자가 "위키에 저장/올려줘"라고 하면 로컬 위키 파일 작성으로 해석한다.
- **지식베이스(KB)**와 **기술자료**는 YouTrack Articles를 뜻하는 동의어다. `DEV2-A-*`, `REF-A-*` 문서는 지식베이스 문서 또는 기술자료라고 부른다.
- YouTrack 지식베이스를 "위키"라고 부르지 않는다. 지식베이스 생성·수정·삭제·이동은 기존 승인 규칙을 그대로 적용한다.
