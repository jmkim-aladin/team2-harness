...
인프라 레벨은 **개발3팀** 영역이다. 개발2팀은 인프라 자원을 직접 만들지 않고 **요청**한다.

| 항목 | 소유 |
|------|------|
| 서버·클러스터·네트워크 생성과 운영 | 개발3팀 |
| 시크릿 매니저 키 생성 | 개발3팀 (개발2팀이 요청) |
| 접근 제어·방화벽 반영 | 개발3팀 (개발2팀이 요청) |
| **구성 설계** (모드·용량·축출·증설 임계·접근 제어 정책) | **개발2팀** |
| **코드 배포** | **개발2팀** |
| **관측 대시보드 설정** | **개발2팀** |
| 반영 후 동작 확인 | 개발2팀 |

- 티켓을 만들 때 **개발3팀 의존 구간은 별도 Task로 분리하고 대기 구간을 본문 When/How에 명시**한다. 시너지팀 QA와 같은 외부 의존 원칙이다 ([주간 핵심 목표 운영 정책](../docs/sprint/weekly-goal-policy.md)).
- 협업 대기 자체는 미달성 사유로 인정되지 않는다. 요청 시점을 앞당기는 것이 개발2팀 몫이다.
- 개발2팀 티켓의 담당자는 **요청·설계·확인을 수행하는 본인**으로 둔다. 개발3팀 작업을 개발2팀 티켓의 담당자로 넣지 않는다.
