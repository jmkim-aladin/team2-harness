...
| 규칙 | 잡는 것 | 등급 |
|---|---|---|
| R1a | 하드룰(반드시·필수·금지…)인데 이유 단서(`근거:`·때문·위해·~므로·~라서·~않으면 등)가 그 item·하위 불릿·**같은 절의 설명 문단·절 제목**에 없고, 조상 preamble에도 명시 표지(`근거:`·`의도:`·`이유:`)가 없음 — 형제 규칙 항목이나 다른 절의 이유로는 면책 안 됨 | block(신규만) |
