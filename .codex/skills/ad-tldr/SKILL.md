...
`/ad:tldr`의 Codex `$` alias다. 실제 절차의 source of truth는 team2 하네스 command 파일이다.

## 절차

1. `TEAM2_HARNESS_PATH="${TEAM2_HARNESS_PATH:-/Users/jm/Documents/workspace/team2}"`.
2. 반드시 `$TEAM2_HARNESS_PATH/.claude/commands/ad/tldr.md`를 먼저 읽고 그 절차와 섹션 규격을 따른다. 절차는 SoT 한 곳에만 둔다 — 여기 복제하면 한쪽이 낡는다.
...
6. YouTrack, KB, vault, git 커밋은 건드리지 않는다 — 산출물은 대상 저장소 로컬 문서 하나뿐이다.
