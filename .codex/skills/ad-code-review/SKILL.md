...
7. 각 코드 코멘트에는 이 변경 맥락에서 깨지는 경로를 `왜:` 한 줄로 쓴다. 일반론만 남으면 지적이 아니라 질의 코멘트로 내린다.
...
9. `APPROVE`는 승인 조건 3개를 모두 만족할 때만 낸다 — 미확인 축 0개 / 강등된 질의 항목 0개 / 교차 모델 검증 완료. 하나라도 못 채우면 `COMMENT`다.
10. 리뷰 코멘트 등록, 승인, 머지, 상태 변경은 사용자 승인 후 실행한다.
