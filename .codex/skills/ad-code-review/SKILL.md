...
11. 미리보기 마지막에 `쉬운 요약 (eli5)` 블록을 붙인다 — `$TEAM2_HARNESS_PATH/.codex/skills/eli5/SKILL.md` §출력 형식의 1·2·5 블록(한 줄 / 구체 사례 / 이것만 기억 3개)만 쓰고, 대상은 이벤트와 전체 메시지뿐이다. 판정을 새로 만들지 않고, 로컬 전용이라 게시 본문에 넣지 않는다.
12. 리뷰 코멘트 등록, 승인, 머지, 상태 변경은 사용자 승인 후 실행한다.
