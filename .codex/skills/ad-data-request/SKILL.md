...
2. 반드시 `$TEAM2_HARNESS_PATH/.claude/commands/ad/data-request.md`를 먼저 읽고 그 절차를 따른다. 절차는 SoT 한 곳에만 둔다 — 여기 복제하면 한쪽이 낡는다.
3. command 파일이 참조하는 데이터 추출 정책과 `AladinCommunication/data-requests-dev2` 기준만 추가로 확인한다.
4. SQL 등록, DB/SP 변경, PR/커밋/푸시는 사용자 승인 후 실행한다.
5. data-requests-dev2 커밋명은 `[DEV2-####] {요청 제목 또는 핵심 산출물 요약}` 형식을 쓴다. 예: `[DEV2-6654] 합산 구매내역 통계 요청`. 기본값으로 `[DEV2-####] 요청 완료`처럼 범용 문구를 쓰지 않는다.
6. 만권당 구독 결제 실제 사용 여부 확인 SQL은 vault `wiki/services/max/analysis/subscription-usage-check-sql.md` 템플릿을 우선 참조한다.
