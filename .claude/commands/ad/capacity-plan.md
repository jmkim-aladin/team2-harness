...
계획 SP 합산 시 **검증(테스트·QA)·배포·운영 반영 Task의 SP를 반드시 포함**한다 ([티켓 가이드 2-2항](../../../docs/sprint/ticket-guide.md)). 개발 Task SP만 세면 수용량이 과대 산정되어 월말 이월로 되돌아온다. Feature에 검증·배포 Task가 없는데 "해당 없음" 사유도 없으면 판정 누락 가능성이 있으므로 계획 검토 시 확인 대상으로 표시한다.
