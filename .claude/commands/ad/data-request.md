...
필요한 것만 읽는다 — 신규 등록(경로·브랜치·커밋 결정)이면 데이터 추출 정책·레포 README, SQL 작성 전 dev DB로 확인할 때는 가설 검증 순서·로컬 자격증명.
...
- **커밋 메시지**: `[DEV2-####] {요청 제목 또는 핵심 산출물 요약}`. 예: `[DEV2-6654] 합산 구매내역 통계 요청`. 상세 규칙은 4단계 참조.
- **경로**:
  - 일반 단건: `요청부서/{부서}/DEV2-####/`
