...
- **커밋 링크는 자동 연동** — YouTrack이 커밋 내역을 자동으로 연결하므로 완료 코멘트에 커밋 URL을 별도로 첨부하지 않는다. 수동 링크 첨부를 위해 push나 링크 제공을 요청하거나 티켓 종료를 보류하지 않는다.
- **기타 결과물** — 지식베이스·문서 등 자동 연동되지 않는 결과물이 있을 때만 해당 링크를 첨부한다. 소스 작업의 커밋 링크를 대체할 목적으로 별도 문서나 PR을 만들지 않는다.
