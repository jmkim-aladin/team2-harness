...
- **단계 혼입** — 개발 Feature의 하위 Task 제목에 `— 테스트` / `— QA` / `— 배포·반영` 구분자가 있으면 위반 후보다. 단계는 Feature로 갈려야 한다
...
- [{idReadable}]({BASE}/issue/{idReadable}) — 단계 혼입: 하위 Task에 {구분자} — {summary} ({담당자})
...
- 카테고리 6은 `type_name == "Feature"` 이고 **제목에 단계 구분자(`— 설계` / `— 테스트` / `— QA` / `— 배포`)가 없을 때만** 평가 (= 개발 Feature)
  ...
  - **단계 혼입**: `subtask_titles` 중 하나라도 단계 구분자를 포함하면 별도 후보로 기록
