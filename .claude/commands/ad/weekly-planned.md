...
로스터 SoT: [policies/team-members.md](../../../policies/team-members.md) — 한글 이름 → YouTrack ID 변환은 그 표를 따른다. 기본 담당자는 김정민(jmkim)·조은흠(heum2).
...
이 스킬은 로컬 위키 저장만 수행한다. YouTrack 티켓·KB 업데이트는 하지 않는다 — 조회 스킬은 부작용이 없어야 반복 실행이 안전하다.
...
`ad:weekly-report` §Feature 기간 초과 경고와 동일 정책 — 해당 섹션(SoT)을 따른다.
