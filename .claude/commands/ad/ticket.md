...
- **Task ≤ 1일 (필수)** — 초과 시 분할
- **Feature 하위 Task 단계 분리 판정 (필수)** — 개발 / 검증(테스트·QA) / 배포·운영 반영. 판정 생략 불가, 해당 시 별도 Task 분리 (아래 "Feature 하위 Task 단계 분리" 절)
...
8. **Feature 기간 검증 (필수)**: 유형이 Feature일 때 아래 검증 수행
   ...
   - 분할 기준: 도메인별 / 단계별(설계 → 개발 → 검증 → 배포·운영 반영) / 의존성 기준
9. **Feature 하위 Task 단계 분리 판정 (필수)**: 아래 "Feature 하위 Task 단계 분리" 절의 절차를 수행한다. Feature 초안을 사용자에게 제시할 때 판정 결과를 함께 출력한다

## Feature 하위 Task 단계 분리 (필수)

SoT: `docs/sprint/ticket-guide.md` 2-2항. Feature를 만들 때 하위 Task를 **작업 단계 기준으로 분리**한다. 하나의 Task에 개발과 검증, 배포·운영 반영을 섞어 담지 않는다.

### 판정 절차

Feature 초안을 확정하기 전에 아래 3단계를 **매번** 판정한다. 판정 자체는 생략할 수 없다.

| 단계 | 판정 질문 | 결과 처리 |
|------|-----------|-----------|
| **개발** | 코드·설정·데이터·SP 변경이 있는가 | 있으면 Task 생성 (사실상 항상) |
| **검증** | 개발 산출물을 별도로 검증해야 하는가. 필요하면 어떤 형태인가 | 필요 → 형태별 Task / 불필요 → 사유 기재 |
| **배포·운영 반영** | 운영 환경 반영과 반영 후 확인이 필요한가 | 필요 → 별도 Task / 불필요 → 사유 기재 |
| (설계·분석) | 구현 전 정책 결정·영향도 분석이 선행되어야 하는가 | 필요할 때만 별도 Task |

- **필요로 판정하면 반드시 별도 Task로 분리한다.** 개발 Task 본문의 체크리스트 항목이나 DoD 항목으로 대체하지 않는다.
- **불필요로 판정하면 Task를 만들지 않는다.** 대신 Feature 본문 `How` 마지막에 `- 검증 해당 없음 — {사유}` / `- 배포·운영 반영 해당 없음 — {사유}` 를 남긴다.
- 판정 근거가 모호하면 사용자에게 **한 번에 하나씩** 묻는다. 추측으로 Task를 만들지 않는다.
...
| **테스트** | 개발2팀 내부 | `— 테스트` 1개 |
| **QA** | 시너지팀 (외부) | `— QA` 1개 |
| **테스트 → QA** | 내부 후 시너지팀 | `— 테스트`, `— QA` **2개로 분리** (순차 의존, 한 Task에 묶지 않음) |

- 대부분의 Feature는 **내부 테스트**로 판정된다. 테스트만으로 충분하면 QA Task를 만들지 않는다.
- **QA는 시너지팀 일정에 의존**하므로 대기 구간이 생긴다. QA Task를 만들 때는 의존 대상과 대기 구간을 본문 `When`/`How`에 명시한다. 협업 대기 자체는 미달성 사유로 인정되지 않는다 (`docs/sprint/weekly-goal-policy.md`).
...
- 개발 Task: `[{서비스}] {업무 요약}` (기존 형식 유지)
- 테스트 Task: `[{서비스}] {업무 요약} — 테스트`
- QA Task: `[{서비스}] {업무 요약} — QA`
- 배포 Task: `[{서비스}] {업무 요약} — 배포·운영 반영`
- SP: 각 Task 독립 산정. 검증·배포 Task를 `0`으로 두지 않는다 (`0`은 Feature 전용)
- 담당자: 테스트는 개발 담당자 또는 팀 내부, **QA는 시너지팀**, 배포·운영 반영은 기획·운영 협업 구간일 수 있다. 서비스 owner를 기본값으로 넣되 다른 담당자가 필요한지 사용자에게 확인한다
- 배포·운영 반영 Task는 프로덕션 배포 사람 승인이 포함되므로 개발 Task 완료만으로 종료 처리하지 않는다
...
Feature 유형이면 5W1H 본문 뒤에 **단계 분리 판정 결과**를 함께 출력한다.
...
| 단계 | 판정 | Task 제목 | 담당 | SP |
|------|------|-----------|------|----|
| 개발 | 필요 | [{서비스}] {업무 요약} | 개발2팀 | 3 |
| 검증 | 필요 — 테스트 | [{서비스}] {업무 요약} — 테스트 | 개발2팀 | 2 |
| 배포·운영 반영 | 해당 없음 | — | — | — |

> 배포·운영 반영 해당 없음 — {사유}
```

검증이 `테스트 → QA`면 두 행으로 나눠 출력한다.

```
| 검증 | 필요 — 테스트 → QA | [{서비스}] {업무 요약} — 테스트 | 개발2팀 | 2 |
| 검증 | 필요 — 테스트 → QA | [{서비스}] {업무 요약} — QA | 시너지팀 | 2 |
```
