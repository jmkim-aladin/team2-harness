...
Hermes Kanban task 본문은 `Source of truth: wiki note`, `Projection: Hermes task`, `Vault path`를 포함한다. 기존 task에는 `TEAM2-SOURCE-LINK` 댓글을 한 번 남겨 같은 연결을 보강한다. task 매핑 state와 action queue는 `source_of_truth`, `vault_path`, `work_id`, `ticket_id`, `service`, `column`을 보존해 모든 지시가 원장 wiki note로 되돌아가게 한다.
...
이 cycle은 dispatch request 생성 뒤 Hermes Kanban `team2` 보드를 자동 동기화하고, 보드 댓글 지시를 action queue로 가져오며, desktop decision cockpit을 갱신한다. 새 decision/review card는 task로 생기고 `blocked` 상태에서 사용자 결정을 기다린다. source card가 해결되어 projection에서 빠지면 연결된 task는 `done`으로 이동한다. task만 직접 완료 처리하지 않고, wiki note의 `decision_status`, `ticket_status`, `review_state`를 먼저 정리한다.
...
Granola 회의록은 별도 no-agent cron인 `team2 granola meeting sync cycle`이 10분마다 `/Users/jm/.hermes-team2/scripts/team2-granola-sync-cycle.sh`를 실행해 처리한다. 이 job은 `tools/run_granola_sync_cycle.py --apply`를 호출하고, 변경 회의록이 없으면 `[SILENT]`만 출력한다. 변경된 회의록에는 티켓/서비스 후보를 `generated:granola-ai-enrichment` block으로만 남기며, transcript 저장, daily note 신규 생성, 결정/confirmed/canonical 승격은 자동으로 하지 않는다.
...
현재 로컬 PGLite 기반 GBrain은 `serve --http` 실행 중에 같은 DB로 `gbrain sync --all`이나 `embed`를 직접 돌리면 lock 경합이 생긴다. 따라서 인덱스 최신화는 Hermes 컨테이너 안에서 Docker socket을 열어 처리하지 않고, host LaunchAgent `com.team2.gbrain-maintenance`가 매일 01:40 KST에 짧게 `gbrain-team2`를 멈춘 뒤 `sync --all`, `sync --source team2-vault --full`, `extract --stale`, `embed --stale`, source별 `dream`, doctor snapshot을 실행하고 다시 올린다. 상태 조회는 `/Users/jm/.hermes-team2/scripts/gbrain-maintenance.sh --status`를 사용한다. 이 조회 모드는 Docker stop이나 maintenance를 실행하지 않는다. Docker runtime은 `GBRAIN_SOURCE=team2-vault`로 두어 업무 원장인 vault를 기본 검색 범위에 둔다. 하네스 검색이 필요하면 명시 source나 로컬 하네스 파일을 사용한다. 장기적으로 무중단 유지보수가 필요하면 GBrain을 Supabase/Postgres/Railway 구성으로 옮긴다.
...
야간 raw 산출물이 `wiki/projects/agentic-os/nightly/`에 남을 때는 `type: project`로 둔다. 서비스 원장으로 승격해 `wiki/services/{service}/analysis/`에 저장하는 분석 노트만 아래 상태로 시작한다.
...
현재 DEV2 공유 gbrain은 Hermes Docker의 `gbrain-team2` 서비스가 제공하는 HTTP MCP를 기준으로 한다. 로컬 Codex/Claude Code는 `http://127.0.0.1:3131/mcp`, Hermes 내부에서는 `http://gbrain-team2:3131/mcp`를 사용한다. Docker runtime은 `GBRAIN_SOURCE=team2-vault`로 실행한다. Mac의 직접 `gbrain` CLI는 개인 로컬 PGLite일 수 있으므로 공유 brain sync, embed, dream 같은 운영 명령은 `docker exec gbrain-team2 ...`로 실행한다.
