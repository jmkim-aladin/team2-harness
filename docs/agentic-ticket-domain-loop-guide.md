...
현재 로컬 PGLite 기반 GBrain은 `serve --http` 실행 중에 같은 DB로 `gbrain sync --all`이나 `embed`를 직접 돌리면 lock 경합이 생긴다. 따라서 인덱스 최신화는 Hermes 컨테이너 안에서 Docker socket을 열어 처리하지 않고, host LaunchAgent `com.team2.gbrain-maintenance`가 매일 01:40 KST에 짧게 `gbrain-team2`를 멈춘 뒤 `sync --all`, `extract --stale`, `embed --stale`, source별 `dream`, doctor snapshot을 실행하고 다시 올린다. 장기적으로 무중단 유지보수가 필요하면 GBrain을 Supabase/Postgres/Railway 구성으로 옮긴다.
