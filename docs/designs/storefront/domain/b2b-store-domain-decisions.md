...
| D-05 | 알림 위치 | 알림 | ✅ **MVP 포함 확정 (4/29)** — 거래성 알림(주문·배송), 배송지 동의 연락처 기반 알림톡 |
...
| D-16 | 고객 CS 관리 환경 (클레임 대응·모니터링) | 주문(클레임)·구매 후 | ✅ **MVP 포함 확정 (4/29)** — 어드민 문의 조회·대응. 채널 결정 후속 |
...
| D-24 | 베스트셀러·신간 데이터 출처 | 전시·카탈로그 | ✅ **알라딘 B2C + 노출범위 필터 확정 (4/29)** |
| D-25 | 주문 상태 운영자 수동 변경 | 주문 | ✅ **MVP 제외 확정 (4/29)** — 시스템 자동 전이만 |
| D-26 | 몰 On/Off (테넌트 접근 제어) | 테넌트 관리 | ✅ **MVP 제외 확정 (4/29)** — Phase 2 |
| D-27 | 배송비 기본 정책 (플랫폼 레벨) | 테넌트 관리·플랫폼 | ✅ **별도 영역 분리 확정 (4/29)** |
...
- **MVP 포함**: 거래성 알림(주문확인·배송) 발송 기능 MVP 편입
- **채널**: 알라딘 알림 시스템 연동 (카카오 알림톡 위주, 어댑터로 격리)
- **트리거**: 각 영역이 도메인 이벤트 발행 → Notification BC가 구독 (5-1 (b)안)
- **SSO 회원 알림 발송 이슈 해결**: SSO 단계에서 연락처를 수집하지 않으므로, **배송지 입력 시 동의받은 연락처(휴대전화·이메일)** 로 알림톡 발송. 구매자가 별도 동의 절차 통과 필요
- **광고성·프로모션 알림**: Phase 2 (5-4, 5-5, 5-6)
- **알림 종류 on/off 정책**: 테넌트 단위 (제휴몰 관리자 R), 회원별 단위는 Phase 2

**파급**:
- service-boundaries.md §2.2 Notification 행 갱신 완료
- 어댑터 위치: `adapter/aladin-noti/` (얇은 OHS)
- 도메인 이벤트 발행자: Order BC(주문확인), Delivery BC(배송 알림)
- 구매자 알림 동의 UX는 결제 직전 배송지 입력 화면에 통합

**참고**: [admin-scope-0423.md §3.2](../admin/b2b-store-admin-scope-0423.md) 법적 검토 결과
...
- **MVP 포함**: 어드민에서 **문의 내용 조회 및 대응** 기능 제공
- **기능 범위 (16-2)**: 문의 접수·답변·응대 이력 관리 (V1.1 정의 그대로). 클레임 연계 포함
- **1차 담당자 (16-1)**: 제휴사 관리자(자사 범위) + 알라딘 CS팀(전체) — RBAC 2-tier
- **채널 구축 방식 (16-3)**: **별도 결정 필요** (자체 구축 vs 알라딘 웹로그인 등 기존 채널 활용). MVP 착수 전 협의
- **통합 조회 범위 (16-4)**: MVP는 클레임 접수 건 + 주문·결제 기본 정보 (감사 로그 D-14 연동)
- **견적 고객 문의 (16-5)**: Phase 2b (견적 몰 함께)

**파급**:
- service-boundaries.md §2.2 CS/고객응대 🟡 → ✅ 승격 완료
- 어드민 UI 구조: 운영 메뉴 안 "CS (문의 조회·대응)" 추가
- A-04 액션 (땡큐 웹로그인 조사) → 채널 결정의 입력으로 사용

**참고**: [admin-scope-0423.md §3.1](../admin/b2b-store-admin-scope-0423.md) 개발 리드 협의 결과
...
## D-24. 베스트셀러·신간 데이터 출처

### 배경

4/29 회의에서 운영자가 "집계 기간 설정"을 요구하려면 SF 자체 주문 데이터 기반이어야 의미 있음. 단, MVP 초기 데이터 부족 문제 — A안(알라딘 기존 데이터) vs B안(SF 자체 집계).

### 결정 (2026-04-29 확정) ✅

- **A안 채택**: **알라딘 B2C 베스트셀러·신간 데이터 활용 + 테넌트별 상품 노출 범위 필터 적용**
- 집계 기간 설정 기능은 **MVP 제외**
- 장기(Phase 2): 운영자가 **베스트셀러 기준 선택** 가능 (알라딘 베스트셀러 / 해당 테넌트 베스트셀러 / 복합 중 1)

### 파급

- Display BC: 자체 집계 로직 X — 알라딘 B2C 데이터 조회 + 노출 범위 필터링만
- 미해결: 노출 범위 필터링 시 응답 시간 (알라딘 B2C 데이터 양 + 필터 비용 검증)
- service-boundaries.md §2.2 Display 행 갱신 완료

**참고**: [admin-scope-0423.md §3.1](../admin/b2b-store-admin-scope-0423.md), 회의 시트2 "검토 필요 항목"

---

## D-25. 주문 상태 운영자 수동 변경

### 배경

현재 SF 주문 상태는 시스템 자동 처리 (saga + 이벤트). 운영자가 수동 개입할 케이스(이상 취소·정책 위반 등) 존재 여부 확인 필요.

### 결정 (2026-04-29 확정) ✅

- **MVP 제외**: 주문 상태 운영자 수동 변경 기능 없음. **시스템 자동 전이만**
- MVP 범위: **주문 조회**만 (목록·상세·결제 내역·클레임 현황)
- 운영자 개입 케이스 정의는 Phase 2

### 파급

- Order BC: 운영자 권한의 상태 변경 API 없음 — 시스템 saga·이벤트만 상태 전이
- 어드민 UI: 주문 상세에서 "조회만, 변경 버튼 없음"
- 이상 케이스(환불 거절·이중 결제 등)는 별도 워크플로우 (manual_refund_queue 등) 통해 처리
- service-boundaries.md §2.2 Order 행 갱신 완료

**참고**: [admin-scope-0423.md §3.1](../admin/b2b-store-admin-scope-0423.md), 회의 시트2 "검토 필요 항목"

---

## D-26. 몰 On/Off (테넌트 접근 제어)

### 배경

기획자 시트 ① 테넌트 설정의 "접근 제어 — 몰 On/Off" 항목. 테넌트 전체 몰을 운영자가 일시 차단할 수 있어야 하는가.

### 결정 (2026-04-29 확정) ✅

- **MVP 제외, 추후 논의**
- 테넌트 단위 비활성화는 Phase 2 (계약 만료·위반·일시 점검 케이스 정리 후)

### 파급

- Tenant Management BC: 테넌트 상태 enum에 ACTIVE / INACTIVE 등 단순 형태만 유지 (운영자 토글 UI 없음)
- 데이터베이스·인증 레벨에서 차단 메커니즘은 Phase 2 설계
- 전용 URL 설정도 함께 Phase 2

**참고**: [admin-scope-0423.md §2.1](../admin/b2b-store-admin-scope-0423.md), 회의 시트1 ① 테넌트 설정

---

## D-27. 배송비 기본 정책 (플랫폼 레벨 별도 영역)

### 배경

테넌트가 배송비를 "유료"로 설정한 경우 적용되는 시스템 기본 정책 — 테넌트 단위가 아닌 **플랫폼 공통**으로 관리되어야 함.

### 결정 (2026-04-29 확정) ✅

- **플랫폼 레벨 별도 관리 영역**으로 분리: 시스템 기본 배송비 정책 (몰타입별)
- 테넌트 배송비 설정에서 "유료" 선택 시 자동 적용
- 적용 범위: 가격, 무료배송 상품 기준 등
- **독립적인 배송비 정책 마련 후 일괄 적용**

### 파급

- Tenant Management BC 안의 별도 영역 (PolicyTemplate / PlatformPolicy 등) 또는 Catalog Overlay 안에 배치
- 어드민 UI: 내부 어드민에 "플랫폼 레벨 설정 > 배송비 기본 정책" 메뉴 분리
- 미해결: 정책 룰 구조(몰타입별 가격 구간·무료배송 상품 기준), 테넌트 정책 vs 플랫폼 정책 충돌 시 우선순위
- service-boundaries.md §2.2 Tenant Management 행 갱신 완료

**참고**: [admin-scope-0423.md §2.4·§5.1](../admin/b2b-store-admin-scope-0423.md), 회의 시트1 🔧 플랫폼 레벨
