...
- lint 베이스라인 (2026-09-16, matcher v8 — 파일럿·전수 판정 후): R1a 76 · R4 43 · R5 4 · R2/R3/R6/R7 0 — 유예 123건, 사유는 `acceptance_batches`에 1회 기록. 대상 128문서(`.codex/skills` 31개 포함 — Codex 파리티) + 카탈로그 14. R1a는 1차 187 → 규칙 강화·Codex 스킬 편입으로 314 → 파일럿 보정으로 237 → 전수 판정(이유 부착·중복 삭제·v8 보정)으로 76. 수치 이동은 전부 측정기 쪽 변화이며 문서가 좋아지거나 나빠진 게 아니다(아래 파일럿)
...
| 유예 | 76 | 자명한 invariant(시크릿·승인 게이트) ~30 · Codex 컨텍스트 스킬의 의도적 복제 18 · 참조·등록부 행 ~15 · **이유를 모르는 것 2**(티켓 본문 코드블록 자제, 신규 .NET 금지의 배경) — 사용자 확인 대기. YouTrack MCP 미사용은 사용자 확인(토큰·쓰기 권한 통제)으로 4곳에 기입 |

결과: R1a 314(v5)→237(v7)→**76**(v8). 유예 367→123. Codex alias 25개에 "절차는 SoT 한 곳에만 — 복제하면 한쪽이 낡는다" 이유 부착.
...
- **이관** R1a 76건 등급 판정 — 회차마다 파일 단위로 훑어 heuristic 완화 / 이유 부착 / `근거:` 부착 중 택일 후 `--update-baseline`. 추이는 아래 거리표 원칙 5 행. 상위 파일: ticket.md·code-review.md·work-prep.md·`.codex/skills/*` 얇은 alias의 "반드시 … 읽고"
...
| 5 | 게이트 기계화 | INVARIANT 중 훅·권한 강제 비율 | 훅 2건 (DB MCP 차단, sqlcmd readonly) | 훅 3건 (+pre-commit: secret_scan·lint ratchet, Claude·Codex 공통). **R1a 근거 미기재 하드룰 76건 유예** (matcher v8 기준, 전수 손판정 후) — 회차마다 이 수를 내린다 |
