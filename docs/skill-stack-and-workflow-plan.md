...
**라우팅은 `description` 한 곳에만 둔다.** `/ad:*`는 모델 호출을 유지하고, CLAUDE.md·AGENTS.md의 라우팅 목록은 제거한다. (2026-08-08 확정)

진단이 처음엔 "자동 호출이 문제"였으나 실측을 다시 보면 **이중화가 문제**였다:

- routing 블록(약 1,400자 ≈ 350 tok)과 각 스킬 description이 **같은 일을 두 번** 하고 있었다. 상주 비용을 두 번 내고, 한쪽이 반드시 낡는다 — 실제로 routing이 없는 `checkpoint` 스킬을 가리키고 있었다
- routing 등록 13개 중 6개는 관측 기간 모델 호출 0회 — 트리거 문구가 약했던 것이지 자동 호출 자체의 문제가 아니다
- routing 블록만 걷고 트리거를 담은 짧은 description(약 540자 ≈ 135 tok)만 남기면 **자동 호출을 유지하면서 215 tok 가벼워진다**

적용:

- `/ad:*` 16종은 **모델 호출** — description에 트리거 문구를 담되 브랜치당 한 문구, 동의어 나열 금지, 한 줄
- **사용자 호출 전용은 사이드이펙트 스킬 4종**(`code-review`·`work-board`·`tldr`·`explain`) — 게시·dispatch는 시점을 사람이 정한다. 2026-07 팀 판정 승계
- CLAUDE.md·AGENTS.md `Skill routing` 절 제거 (문서 위치 결정 트리처럼 스킬이 아닌 규칙은 유지)
- superpowers SessionStart 강제 주입 무력화 — 이건 별개 사안이다. "1% 확률이면 스킬 호출" 같은 매 턴 스캔 강제가 지연을 만들지, 스킬이 모델 호출인 것 자체는 문제가 아니다
- **flow 지도**를 `docs/harness-guide.md` 한 절로 둔다 — 자동 호출이 있어도 사람이 플로우 전체를 보는 지도는 필요하다 (원문 `ask-matt` 라우터의 변형 채택)

> **부작용 확인**: 사용자 호출 전용으로 만들면 description이 모델 시야에서 빠지므로 **다른 스킬도 그 스킬을 부를 수 없다.** `ad:*` 중 다른 `ad:*`를 호출하는 스킬은 0건이라 4종을 사용자 호출로 두어도 영향이 없다.
