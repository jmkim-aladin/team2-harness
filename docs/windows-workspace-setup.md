...
Windows에서도 macOS와 같은 팀 하네스, 위키, 서비스 레포 구성을 쓰기 위한 절차이다.
...
이미 클론된 레포는 삭제하거나 덮어쓰지 않고 `core.longpaths=true` 같은 Windows용 Git 옵션만 보강한다.

사용자 환경변수는 새 터미널과 새 앱 세션부터 자동 반영된다.

## 레포만 다시 동기화

하네스 카탈로그에 레포가 추가되었을 때는 아래 명령만 다시 실행한다.
...
다음 레포는 히스토리에 Windows 파일시스템에서 허용하지 않는 경로가 있어 sparse checkout 예외를 적용한다.

- `Ebook_Web_Viewer`: literal backslash가 포함된 `logs\...` 경로 제외
- `EbookIos`: `:`가 포함된 HTML 리소스 1개 제외

이 예외는 Windows 작업 트리를 깨끗하게 유지하기 위한 것이며, Git 오브젝트와 원격 히스토리는 그대로 유지된다. 해당 파일을 실제로 수정해야 하는 경우 macOS 환경에서 작업한다.

## wiki 참조 shopping 레포

워크플로우 문서에서 직접 참조하는 shopping 레포도 같은 묶음에 둔다.

- `C:\Users\jmkim\Documents\workspace\shopping\dev1-web-aladin`
- `C:\Users\jmkim\Documents\workspace\shopping\mall-search`

`shop-db-script`는 위키에 경로가 남아 있지만 현재 `https://github.com/AladinCommunication/shop-db-script` 원격을 찾을 수 없어 자동 클론 대상에서 제외했다.

## macOS와 Windows 공통 운영 원칙

- 하네스 정책과 스킬은 `TEAM2_HARNESS_PATH`를 기준으로 참조한다.
- 서비스 작업은 각 서비스 레포에서 실행한다.
- 하네스/정책/워크플로우 수정은 `team2-harness`에서 관리한다.
- 위키 자료는 workspace의 `wiki` 레포를 기준으로 동기화한다.
